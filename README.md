...
- **Звонки** и **Классификация**: серверная **пагинация** (`offset` + `limit`), ссылки «Назад / Вперёд»; «Применить фильтры» сбрасывает страницу (`offset=0`). Лимит по умолчанию 1000, верхняя граница завышена для больших выборок. Полная выгрузка — через Excel. Остальные страницы (пользователи, логи) по-прежнему одной страницей с лимитом.
- фильтры:
