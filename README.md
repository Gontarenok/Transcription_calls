...
- **Звонки** и **Классификация**: серверная **пагинация** фиксированными **1000 строк на страницу** (поле «Лимит» в UI нет), ссылки «Назад / Вперёд», счётчик «Всего». При очень большом числе записей по фильтру (порог в коде, сейчас 50 000) показывается жёлтое предупреждение. Полная выгрузка — через Excel. JSON API `/api/calls` по-прежнему принимает `limit`/`offset`. Остальные страницы (пользователи, логи) — одной выборкой с лимитом.
- фильтры:
